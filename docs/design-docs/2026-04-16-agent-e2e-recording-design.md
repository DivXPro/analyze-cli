...
构建一套可复用的 Agent 驱动录制机制，使 AI Agent 能够通过 OpenCLI 获取真实平台数据，并调用 `analyze-cli` CLI 完成数据导入。同时，将本次在线交互中获取的原始数据自动保存为 fixture，并生成对应的离线测试文件，供后续 CI 和回归测试使用。
...
- 未来真实场景是：**Agent 调用 OpenCLI → Agent 调用 analyze-cli CLI → 数据进入数据库**。本设计旨在让测试体系与这一工作流对齐。
...
- 通过 `child_process.spawn` 或 `execFile` 调用 `analyze-cli` 命令
...
**内部流程**
1. **创建 platform**：调用 `analyze-cli platform add --id <id> --name <name>`
2. **搜索帖子**：调用 `opencli xiaohongshu search <query> --limit <n> -f json`，保存原始响应为 `<output>/posts_raw.json`
3. **导入帖子**：调用 `analyze-cli post import --platform <id> --file <output>/posts_raw.jsonl`
...
6. **导入评论**：调用 `analyze-cli comment import --post-id <id> --file ...`
...
└── 2026-04-16-xhs-shanghai/
    ...
    ├── posts_transformed.jsonl # 适配 analyze-cli post import 的格式
...
- 不修改 `analyze-cli` 核心 CLI 命令的签名或行为
- 不在 `analyze-cli` 运行时内置 fixture 录制逻辑
- 不引入新的测试框架，继续使用 `node:test`

## 验收标准

- [ ] 录制脚本可成功从 OpenCLI 获取 XHS 帖子、评论、媒体数据
- [ ] 录制脚本成功调用 `analyze-cli` CLI 完成所有导入操作
