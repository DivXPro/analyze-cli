...
| # | 工具 | 命令 | 使用场景 |
|---|------|------|----------|
| 1 | **search_posts** | `opencli <site> <command> {query} --limit {limit} -f json` | 导入前发现帖子。**各平台命令不同** — (1) 用 `opencli list | grep <keyword>` 找平台，(2) 用 `opencli <platform> --help` 查看可用命令，(3) 用 `opencli <platform> <command> -h` 了解具体用法。例如：抖音用 `hashtag` 而不是 `search`。 |
